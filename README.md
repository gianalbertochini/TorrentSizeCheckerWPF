...
>
> 🟢 **No installation required. No .NET runtime required.** The release is a fully self-contained single `.exe` file — just download and run.
...
- ✅ **Standalone executable — no .NET installation required, no external DLLs**
...
- Windows 10 / 11 (x64)
- **No .NET runtime installation required** — the `.exe` is self-contained and bundles the runtime inside

> ℹ️ The executable is larger than a typical app (~120 MB) because it includes the full .NET 10 runtime and all dependencies. This is intentional: the user does not need to install anything.
...
1. Launch `TorrentSizeChecker.exe` (no installation needed, just double-click).
